					   ...
					   BlockNumber blkno, OffsetNumber offno)
{
	...
	HnswInitNeighbors(NULL, element, disk->meta.m, HnswGetAcornBuild(disk->index), disk->allocator);
	if (etup->deleted)
	{
					  ...
					  HnswNeighborTuple ntup, Size itemSize)
{
	...
	bool		acornBuild = HnswGetAcornBuild(disk->index);
	int			expected = owner->level * disk->meta.m + HnswGetLayerM(disk->meta.m, 0, acornBuild);
	int			idx = 0;
	char	   *base = NULL;

	if (itemSize != HnswNeighborTupleSize(owner->level, disk->meta.m, acornBuild) ||
		ntup->count != expected)
		HnswCloneCorruption(disk->index,
	...
	for (int lc = owner->level; lc >= 0; lc--)
	{
		...
		int			lm = HnswGetLayerM(disk->meta.m, lc, acornBuild);
		bool		seenInvalid = false;
