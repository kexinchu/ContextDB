...
HnswInit(void)
{
	...
	add_bool_reloption(hnsw_relopt_kind, "acorn", "Build level-0 graph with ACORN-1 construction shrink",
				 ...
				 double *indexPages)
{
	...
	if (path->indexinfo->tuples > 0)
	{
		...
		int			layer0TuplesMax = HnswGetLayerM(m, 0, false) * hnsw_ef_search;
...
hnswoptions(Datum reloptions, bool validate)
{
	static const relopt_parse_elt tab[] = {
		...
		{"acorn", RELOPT_TYPE_BOOL, offsetof(HnswOptions, acorn)},
	};
