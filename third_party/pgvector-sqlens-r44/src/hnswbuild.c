...
CreateGraphPages(HnswBuildState * buildstate)
{
	...
	while (!HnswPtrIsNull(base, iter))
	{
		...
		ntupSize = HnswNeighborTupleSize(element->level, buildstate->m, buildstate->acornBuild);
...
WriteNeighborTuples(HnswBuildState * buildstate)
{
	...
	while (!HnswPtrIsNull(base, iter))
	{
		...
		Buffer		buf;
		Page		page;
		Size		ntupSize = HnswNeighborTupleSize(element->level, m, buildstate->acornBuild);
		...
		HnswSetNeighborTuple(base, ntup, element, m, buildstate->acornBuild);
 ...
 */
static void
UpdateNeighborsInMemory(char *base, HnswSupport * support, HnswElement e, int m, bool acornBuild)
{
	for (int lc = e->level; lc >= 0; lc--)
	{
		...
		for (int i = 0; i < neighbors->length; i++)
		{
			...
			HnswUpdateConnection(base, HnswGetNeighbors(base, neighborElement, lc), e, hc->distance, lm, NULL, NULL, support, acornBuild, lc);
...
UpdateGraphInMemory(HnswSupport * support, HnswElement element, int m, HnswElement entryPoint, HnswBuildState * buildstate)
{
	...
	/* Update neighbors */
	UpdateNeighborsInMemory(base, support, element, m, buildstate->acornBuild);
...
InsertTupleInMemory(HnswBuildState * buildstate, HnswElement element)
{
	...
	HnswFindElementNeighbors(base, element, entryPoint, NULL, support, m, efConstruction, false, buildstate->acornBuild);
...
InsertTuple(Relation index, Datum *values, bool *isnull, ItemPointer heaptid, HnswBuildState * buildstate)
{
	...
	element = HnswInitElement(base, heaptid, buildstate->m, buildstate->ml, buildstate->maxLevel, buildstate->acornBuild, allocator);
...
InitBuildState(HnswBuildState * buildstate, Relation heap, Relation index, IndexInfo *indexInfo, ForkNumber forkNum)
{
	...
	buildstate->acornBuild = HnswGetAcornBuild(index);
