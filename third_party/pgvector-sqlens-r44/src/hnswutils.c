...
HnswHybridAdmitFiltered(char *base, HnswQuery * q, ItemPointer indextid, Relation index,
						...
						pairingheap *C, pairingheap *W, int *wlen, int ef,
...
HnswHybridExpandTwoHop(char *base, HnswQuery * q, ItemPointer bridgeTid, Relation index,
					   ...
					   HnswScanGuidance *guidance, visited_hash * v, pairingheap *C,
					   ...
					   ItemPointerData *n2)
{
	...
	for (int k = 0; k < lm; k++)
	{
		...
		if (HnswHybridAdmitFiltered(base, q, &n2[k], index, support, guidance, v, C, W,
				   ...
				   HnswScanGuidance *guidance, HnswIndexPageProfileState *indexPageProfile)
{
	...
	while (!pairingheap_is_empty(C))
	{
		...
		for (int j = 0; j < lm; j++)
		{
			...
			if (HnswHybridAdmitFiltered(base, q, &n1[j], index, support, guidance, v, C, W,
										&wlen, ef, indexPageProfile, distanceComputations,
										traversalProfile))
			{
				numFound++;
				if (numFound >= m2)
...
SelectNeighborsAcorn(char *base, List *c, int maxSize, int mBeta, int m, HnswSupport *support)
{
	...
	foreach(lc, c)
	{
		HnswCandidate *v1 = (HnswCandidate *) lfirst(lc);
