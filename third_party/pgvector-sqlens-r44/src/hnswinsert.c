...
AddElementOnDisk(Relation index, HnswElement e, int m, BlockNumber insertPage, BlockNumber *updatedInsertPage, bool building)
{
	...
	ntupSize = HnswNeighborTupleSize(e->level, m, acornBuild);
	combinedSize = etupSize + ntupSize + sizeof(ItemIdData);
	maxSize = HNSW_MAX_SIZE;
	minCombinedSize = etupSize + HnswNeighborTupleSize(0, m, acornBuild) + sizeof(ItemIdData);
	...
	HnswSetNeighborTuple(base, ntup, e, m, acornBuild);
...
GetUpdateIndex(HnswElement element, HnswElement newElement, float distance, int m, int lm, int lc, Relation index, HnswSupport * support, MemoryContext updateCtx)
{
	...
	else
	{
		...
		if (idx == -1)
			HnswUpdateConnection(base, neighbors, newElement, distance, lm, &idx, index, support, HnswGetAcornBuild(index), lc);
	}
...
HnswUpdateNeighborsOnDisk(Relation index, HnswSupport * support, HnswElement e, int m, bool checkExisting, bool building)
{
	...
	{
		int			lm = HnswGetLayerM(m, lc, HnswGetAcornBuild(index));
...
HnswInsertTupleOnDisk(Relation index, HnswSupport * support, Datum value, ItemPointer heaptid, bool building)
{
	...
	/* Create an element */
	element = HnswInitElement(base, heaptid, m, HnswGetMl(m), HnswGetMaxLevel(m), HnswGetAcornBuild(index), NULL);
	...
	HnswFindElementNeighbors(base, element, entryPoint, index, support, m, efConstruction, false, HnswGetAcornBuild(index));
