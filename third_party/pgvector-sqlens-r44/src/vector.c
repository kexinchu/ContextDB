...
#define HNSW_ADAPTIVE_PROBE_MAX_TIDS 256
#define HNSW_ADAPTIVE_BLOOM_FP 0.01
...
typedef struct HnswGuidanceDescriptorEntry
{
	...
	ItemPointerData adaptiveProbeTids[HNSW_ADAPTIVE_PROBE_MAX_TIDS];
	int			adaptiveProbeTidCount;
...
static bool hnsw_d3_compose_complete_only = true;
static double hnsw_d3_page_max_regret = 0.10;
static double hnsw_d3_min_marginal_skip = 0.02;
static double hnsw_d3_lambda = 0;
static double hnsw_d3_expected_reuse = 8;
static bool hnsw_page_compose_upgraded = false;
static double hnsw_last_page_regret = 0;
static double hnsw_last_marginal_delta = 0;
...
_PG_init(void)
{
	...
	DefineCustomRealVariable("hnsw.d3_min_benefit_per_byte",
							 "Legacy benefit-per-byte cutoff. Unused by ChooseKind and Admit.",
							 "Kept so older sessions can set it.",
							 &hnsw_d3_min_benefit_per_byte,
							 0, 0, DBL_MAX, PGC_USERSET, 0, NULL, NULL, NULL);

	DefineCustomIntVariable("hnsw.d3_max_fragment_mb",
							"Sets the per-atom fragment size cap B_atom",
							"An atom is rejected when its chosen payload would exceed this cap.",
							&hnsw_d3_max_fragment_mb,
							16, 1, 1024 * 1024, PGC_USERSET, 0, NULL, NULL, NULL);

	DefineCustomRealVariable("hnsw.d3_page_min_skip_rate",
							 "Legacy page-skip cutoff. Unused when compose-complete mode is on.",
							 "Kept so older sessions can set it. Refinement uses hnsw.d3_page_max_regret.",
							 &hnsw_d3_page_min_skip_rate,
								 0.05, 0, 1, PGC_USERSET, 0, NULL, NULL, NULL);
	DefineCustomBoolVariable("hnsw.d3_compose_complete_only",
							 "Keeps page bitmaps out of multi-atom guides",
							 "A page bitmap cannot prove every ineligible TID, so two page atoms are upgraded to Bloom.",
							 &hnsw_d3_compose_complete_only,
							 true, PGC_USERSET, 0, NULL, NULL, NULL);
	DefineCustomRealVariable("hnsw.d3_page_max_regret",
							 "Maximum exact-minus-page skip allowed before a unary page fragment is refined",
							 "A unary page bitmap stays only when it is already close to an exact set.",
							 &hnsw_d3_page_max_regret,
							 0.10, 0, 1, PGC_USERSET, 0, NULL, NULL, NULL);
	DefineCustomRealVariable("hnsw.d3_min_marginal_skip",
							 "Legacy independence-proxy cutoff. Unused by Admit.",
							 "Kept so older sessions can set it.",
							 &hnsw_d3_min_marginal_skip,
							 0.02, 0, 1, PGC_USERSET, 0, NULL, NULL, NULL);
	DefineCustomRealVariable("hnsw.d3_lambda",
							 "Memory weight in ChooseKind and Admit",
							 "Multiplies payload bytes in both scores. Zero ignores memory except the hard caps.",
							 &hnsw_d3_lambda,
							 0, 0, DBL_MAX, PGC_USERSET, 0, NULL, NULL, NULL);
	DefineCustomRealVariable("hnsw.d3_expected_reuse",
							 "Expected later scans that imply the atom",
							 "The r-hat term in ChooseKind and Admit.",
							 &hnsw_d3_expected_reuse,
							 8, 1, 1e6, PGC_USERSET, 0, NULL, NULL, NULL);
							...
							bool tracked, int64 epoch, Oid relFileNode)
{
	...
	descriptor->adaptiveRelFileNode = relFileNode;
	descriptor->adaptiveProbeTidCount = 0;
}
...
static int
HnswGuidanceCountAtoms(Datum *filterDatums, bool *filterNulls, int filterCount)
{
	int			atoms = 0;
...
HnswGuidanceApplyComposeKind(HnswGuidanceKind kind, int atomCount)
{
	if (hnsw_d3_compose_complete_only &&
		atomCount > 1 &&
		kind == HNSW_GUIDANCE_KIND_PAGE)
	{
		hnsw_page_compose_upgraded = true;
...
void
HnswGuidanceRecordProbeTid(ItemPointer tid)
{
	HnswGuidanceDescriptorEntry *descriptor = hnsw_adaptive_probe.descriptor;

	if (descriptor == NULL || tid == NULL || !ItemPointerIsValid(tid))
		return;
	if (descriptor->adaptiveProbeTidCount >= HNSW_ADAPTIVE_PROBE_MAX_TIDS)
		return;
	if (descriptor->adaptiveProbeTidCount > 0 &&
		ItemPointerEquals(&descriptor->adaptiveProbeTids[descriptor->adaptiveProbeTidCount - 1], tid))
		return;
	descriptor->adaptiveProbeTids[descriptor->adaptiveProbeTidCount++] = *tid;
}
...
HnswGuidanceValidateAdaptiveAtoms(Datum *filterDatums, bool *filterNulls, int filterCount)
{
	...
	if (exactAtoms)
		return HNSW_GUIDANCE_KIND_EXACT;
	if (hnsw_d3_compose_complete_only && atoms > 1)
		return HNSW_GUIDANCE_KIND_BLOOM;
	return HNSW_GUIDANCE_KIND_PAGE;
}

typedef struct HnswAdaptiveAtomPlan
{
	char	   *filterName;
	bool		admitted;
	HnswGuidanceKind kind;
	...
	double		deltaAlpha;
	int64		bytes;
} HnswAdaptiveAtomPlan;

static double
HnswAdaptiveMeanFetchMs(HnswGuidanceDescriptorEntry *descriptor)
{
	double		ms = 0;

	if (descriptor->adaptiveCycleProbes > 0)
	{
		ms = descriptor->adaptiveProbeHeapFetchMs / descriptor->adaptiveCycleProbes;
		if (ms <= 0)
			ms = descriptor->adaptiveProbeTotalMs / descriptor->adaptiveCycleProbes;
	}
	return ms > 1.0 ? ms : 1.0;
}

static double
HnswAdaptiveKindCostMs(HnswGuidanceKind kind, double fetchMs)
{
	switch (kind)
	{
		case HNSW_GUIDANCE_KIND_PAGE:
...
static int64
HnswAdaptiveEstimateKindBytes(Oid heapOid, HnswGuidanceKind kind, int64 matchingRows)
{
	Relation	heapRel = table_open(heapOid, AccessShareLock);
	...
	if (kind == HNSW_GUIDANCE_KIND_BLOOM)
	{
		uint64		bloomBits = Max((uint64) 1024, (uint64) Max(matchingRows, 1) * 10);

		return (int64) ((bloomBits + 7) / 8);
	}
	return Max(matchingRows, 1) * (int64) sizeof(HnswMetadataTidEntry);
}

static HnswGuidanceKind
HnswAdaptiveCacheKind(HnswMetadataCacheEntry *cache)
{
	if (cache == NULL)
		return HNSW_GUIDANCE_KIND_OFF;
	if (cache->adaptiveKind == HNSW_GUIDANCE_KIND_EXACT ||
		cache->adaptiveKind == HNSW_GUIDANCE_KIND_PAGE ||
		cache->adaptiveKind == HNSW_GUIDANCE_KIND_BLOOM)
		return (HnswGuidanceKind) cache->adaptiveKind;
	if (cache->tidHash != NULL)
		return HNSW_GUIDANCE_KIND_EXACT;
	if (cache->bloomBits != NULL)
		return HNSW_GUIDANCE_KIND_BLOOM;
...
static void
HnswAdaptiveResidentSkipsOnTids(Oid heapOid, const char *excludeName,
								const ItemPointerData *tids, int n, bool *skip)
{
	HASH_SEQ_STATUS status;
	HnswMetadataCacheEntry *entry;

	memset(skip, 0, sizeof(bool) * n);
	if (hnsw_metadata_caches == NULL || n <= 0)
		return;
	hash_seq_init(&status, hnsw_metadata_caches);
	while ((entry = (HnswMetadataCacheEntry *) hash_seq_search(&status)) != NULL)
	{
		...
		if (entry->key.heapOid != heapOid)
			continue;
		if (excludeName != NULL && strcmp(entry->key.filterName, excludeName) == 0)
			continue;
		kind = HnswAdaptiveCacheKind(entry);
		if (kind == HNSW_GUIDANCE_KIND_OFF)
			continue;
		for (int i = 0; i < n; i++)
		{
			if (!skip[i] && !HnswGuidanceCacheAllowsTid(entry, kind, (ItemPointer) &tids[i]))
				skip[i] = true;
		}
	}
}
...
static bool
HnswAdaptiveEvalAtomOnTids(Oid heapOid, const char *filterName,
						   const ItemPointerData *tids, int n,
						   bool *failExact, bool *failPage,
						   double *alphaExact, double *alphaPage)
{
	StringInfoData sql;
	char	   *qualifiedName;
	const char *tidColumn;
	const char *predicate;
	int			spiStatus;
	BlockNumber blocks[HNSW_ADAPTIVE_PROBE_MAX_TIDS];
	int			nblocks = 0;
	int			exactFails = 0;
	int			pageFails = 0;
	...
	*alphaPage = 0;
	...
	qualifiedName = HnswMetadataQualifiedSource(heapOid, &tidColumn);
	initStringInfo(&sql);
	appendStringInfo(&sql, "SELECT %s FROM %s WHERE (%s) AND %s = ANY (ARRAY[",
					 tidColumn, qualifiedName, predicate, tidColumn);
	for (int i = 0; i < n; i++)
	{
		if (i > 0)
			appendStringInfoChar(&sql, ',');
		appendStringInfo(&sql, "'(%u,%u)'",
						 ItemPointerGetBlockNumber(&tids[i]),
						 ItemPointerGetOffsetNumber(&tids[i]));
	}
	appendStringInfoString(&sql, "]::tid[])");
	...
	{
		bool		isnull;
		Datum		ctidDatum = SPI_getbinval(SPI_tuptable->vals[i],
											  SPI_tuptable->tupdesc, 1, &isnull);
		ItemPointer ctid;
		...
		for (int j = 0; j < n; j++)
		{
			if (ItemPointerEquals((ItemPointer) &tids[j], ctid))
				failExact[j] = false;
		}
	}
	SPI_finish();
	...
	for (int i = 0; i < n; i++)
	{
		...
		if (!seen && nblocks < HNSW_ADAPTIVE_PROBE_MAX_TIDS)
	...
	for (int b = 0; b < nblocks; b++)
	{
		...
		resetStringInfo(&sql);
		appendStringInfo(&sql,
						 "SELECT 1 FROM %s WHERE (%s) AND %s >= '(%u,1)' AND %s < '(%u,1)' LIMIT 1",
						 qualifiedName, predicate, tidColumn, blocks[b],
						 tidColumn, blocks[b] + 1);
		spiStatus = SPI_connect();
		if (spiStatus != SPI_OK_CONNECT)
			elog(ERROR, "SPI_connect failed: %d", spiStatus);
		spiStatus = SPI_execute(sql.data, true, 1);
		...
		for (int i = 0; i < n; i++)
		{
			if (ItemPointerGetBlockNumber(&tids[i]) == blocks[b])
				failPage[i] = false;
		}
	}
	...
	*alphaExact = (double) exactFails / n;
	*alphaPage = (double) pageFails / n;
	pfree(sql.data);
	return true;
}

static HnswGuidanceKind
HnswAdaptiveChooseKind(int statementAtoms, bool forceExact,
					   double alphaExact, double alphaPage,
					   double fetchMs, double reuse,
					   int64 bytesExact, int64 bytesPage, int64 bytesBloom,
					   int64 atomCap)
{
	HnswGuidanceKind bestKind = HNSW_GUIDANCE_KIND_OFF;
	double		bestScore = -DBL_MAX;
	double		alphaBloom = alphaExact * (1.0 - HNSW_ADAPTIVE_BLOOM_FP);
	bool		allowPage = !forceExact &&
		(statementAtoms <= 1 || !hnsw_d3_compose_complete_only);

	if (forceExact && bytesExact <= atomCap && bytesExact > 0)
		return HNSW_GUIDANCE_KIND_EXACT;

	if (allowPage && bytesPage > 0 && bytesPage <= atomCap)
	{
		double		score = alphaPage * fetchMs -
			HnswAdaptiveKindCostMs(HNSW_GUIDANCE_KIND_PAGE, fetchMs) / reuse -
			hnsw_d3_lambda * (double) bytesPage;

		bestKind = HNSW_GUIDANCE_KIND_PAGE;
		bestScore = score;
	}
	if (bytesBloom > 0 && bytesBloom <= atomCap)
	{
		double		score = alphaBloom * fetchMs -
			HnswAdaptiveKindCostMs(HNSW_GUIDANCE_KIND_BLOOM, fetchMs) / reuse -
			hnsw_d3_lambda * (double) bytesBloom;

		if (score > bestScore)
		{
			bestKind = HNSW_GUIDANCE_KIND_BLOOM;
			bestScore = score;
		}
	}
	if (bytesExact > 0 && bytesExact <= atomCap)
	{
		double		score = alphaExact * fetchMs -
			HnswAdaptiveKindCostMs(HNSW_GUIDANCE_KIND_EXACT, fetchMs) / reuse -
			hnsw_d3_lambda * (double) bytesExact;

		if (score > bestScore)
		{
			bestKind = HNSW_GUIDANCE_KIND_EXACT;
			bestScore = score;
		}
	}
	return bestKind;
...
static int
HnswAdaptivePlanAndAdmit(Oid heapOid, Datum *filterDatums, bool *filterNulls,
						 int filterCount, HnswGuidanceDescriptorEntry *descriptor,
						 bool forceExact, HnswAdaptiveAtomPlan *plans, int *nPlans)
{
	int			n = 0;
	int			admitted = 0;
	int			probeN = descriptor->adaptiveProbeTidCount;
	const ItemPointerData *tids = descriptor->adaptiveProbeTids;
	bool		admittedSkip[HNSW_ADAPTIVE_PROBE_MAX_TIDS];
	double		fetchMs = HnswAdaptiveMeanFetchMs(descriptor);
	double		reuse = hnsw_d3_expected_reuse;
	int64		atomCap = HnswAdaptiveFragmentLimitBytes();
	int64		globalCap = (int64) hnsw_metadata_cache_max_mb * 1024L * 1024L;
	int64		residentBytes = HnswMetadataCacheTotalBytes();
	int			statementAtoms;
	...
	*nPlans = 0;
	for (int i = 0; i < filterCount; i++)
	{
		...
		plans[n].filterName = filterName;
		plans[n].admitted = false;
		plans[n].kind = HNSW_GUIDANCE_KIND_OFF;
		plans[n].alphaExact = 0;
		plans[n].alphaPage = 0;
		plans[n].deltaAlpha = 0;
		plans[n].bytes = 0;
		n++;
	}
	*nPlans = n;
	...
	memset(admittedSkip, 0, sizeof(admittedSkip));

	for (int a = 0; a < n; a++)
	{
		bool		failExact[HNSW_ADAPTIVE_PROBE_MAX_TIDS];
		bool		failPage[HNSW_ADAPTIVE_PROBE_MAX_TIDS];
		bool		residentSkip[HNSW_ADAPTIVE_PROBE_MAX_TIDS];
		bool		composeSkip[HNSW_ADAPTIVE_PROBE_MAX_TIDS];
		int64		matchingRows;
		int64		bytesExact;
		int64		bytesPage;
		int64		bytesBloom;
		...
		memset(residentSkip, 0, sizeof(residentSkip));
		if (probeN > 0)
		{
			HnswAdaptiveResidentSkipsOnTids(heapOid, plans[a].filterName,
											tids, probeN, residentSkip);
			HnswAdaptiveEvalAtomOnTids(heapOid, plans[a].filterName, tids, probeN,
									   failExact, failPage,
									   &plans[a].alphaExact, &plans[a].alphaPage);
			for (int t = 0; t < probeN; t++)
				composeSkip[t] = residentSkip[t] || admittedSkip[t];
		}
		else
		{
			plans[a].alphaExact = 0;
			plans[a].alphaPage = 0;
		}
		...
		if (probeN > 0)
		{
			...
			for (int t = 0; t < probeN; t++)
			{
				if (composeSkip[t])
					skipBefore++;
				if (composeSkip[t] || failExact[t])
					skipAfter++;
			}
			delta = ((double) skipAfter / probeN) - ((double) skipBefore / probeN);
		}
		else
			delta = plans[a].alphaExact;

		plans[a].deltaAlpha = delta;
		matchingRows = 1;
		heapRel = table_open(heapOid, AccessShareLock);
		totalRows = heapRel->rd_rel->reltuples;
		table_close(heapRel, AccessShareLock);
		if (totalRows > 0)
			matchingRows = Max((int64) 1,
				(int64) (totalRows * (1.0 - plans[a].alphaExact)));

		bytesExact = HnswAdaptiveEstimateKindBytes(heapOid, HNSW_GUIDANCE_KIND_EXACT, matchingRows);
		bytesPage = HnswAdaptiveEstimateKindBytes(heapOid, HNSW_GUIDANCE_KIND_PAGE, matchingRows);
		bytesBloom = HnswAdaptiveEstimateKindBytes(heapOid, HNSW_GUIDANCE_KIND_BLOOM, matchingRows);
		kind = HnswAdaptiveChooseKind(statementAtoms, forceExact,
									  plans[a].alphaExact, plans[a].alphaPage,
									  fetchMs, reuse, bytesExact, bytesPage,
									  bytesBloom, atomCap);
		if (kind == HNSW_GUIDANCE_KIND_OFF)
			continue;
		if (kind == HNSW_GUIDANCE_KIND_PAGE)
		{
			alphaKind = plans[a].alphaPage;
			kindBytes = bytesPage;
			if (probeN > 0)
			{
				int			skipBefore = 0;
		...
		{
			alphaKind = plans[a].alphaExact * (1.0 - HNSW_ADAPTIVE_BLOOM_FP);
			kindBytes = bytesBloom;
			delta *= (1.0 - HNSW_ADAPTIVE_BLOOM_FP);
			plans[a].deltaAlpha = delta;
		}
		else
		{
			alphaKind = plans[a].alphaExact;
			kindBytes = bytesExact;
		}
		(void) alphaKind;
		costMs = HnswAdaptiveKindCostMs(kind, fetchMs);
		if (delta * fetchMs * reuse < costMs + hnsw_d3_lambda * (double) kindBytes)
			continue;
		if (kindBytes > atomCap || residentBytes + kindBytes > globalCap)
			continue;

		plans[a].admitted = true;
		plans[a].kind = kind;
		plans[a].bytes = kindBytes;
		residentBytes += kindBytes;
		admitted++;
		if (probeN > 0)
		{
			for (int t = 0; t < probeN; t++)
		...
		hnsw_last_marginal_delta = delta;
	}
							 ...
							 Expr *predicate)
{
	...
	HnswAdaptiveAtomPlan atomPlans[HNSW_GUIDANCE_MAX_ATOMS];
	int			nAtomPlans = 0;
	int			planIndex = 0;
	bool		refinement = false;
	bool		reactivation = descriptor->adaptiveAdmissions > 0 &&
	...
	if (!reactivation)
	{
		if (HnswAdaptivePlanAndAdmit(heapOid, filterDatums, filterNulls,
				filterCount, descriptor, stage == HNSW_GUIDANCE_KIND_EXACT,
				atomPlans, &nAtomPlans) <= 0)
			return 0;
	}
	else if (stage != HNSW_GUIDANCE_KIND_EXACT &&
	...
	for (int i = 0; i < filterCount; i++)
	{
		char	   *filterName;
		HnswGuidanceKind ignoredKind;
		HnswGuidanceKind atomKind = stage;
		bool		cacheHit = false;
		bool		storeHit = false;
		HnswMetadataCacheEntry *cache;
		int		atomIndex;
		...
		if (!reactivation)
		{
			if (planIndex >= nAtomPlans || !atomPlans[planIndex].admitted)
			{
				planIndex++;
				continue;
			}
			atomKind = atomPlans[planIndex].kind;
			planIndex++;
		}
		if (atomKind == HNSW_GUIDANCE_KIND_EXACT)
			cache = GetHnswMetadataCache(heapOid, filterName, true, false,
									 &cacheHit, &storeHit);
		else if (atomKind == HNSW_GUIDANCE_KIND_PAGE)
		...
		if (atomIndex == 0)
			nextGuidance.kind = atomKind;
		cache->adaptiveManaged = true;
		cache->adaptiveKind = atomKind;
		fragmentBytes += HnswMetadataCacheMemoryBytes(cache, atomKind);
		nextGuidance.lastCacheRows += atomKind == HNSW_GUIDANCE_KIND_EXACT ?
			cache->rows : (atomKind == HNSW_GUIDANCE_KIND_PAGE ? cache->pageRows : cache->bloomRows);
		if (atomKind == HNSW_GUIDANCE_KIND_PAGE)
		...
		nextGuidance.lastBuildMs += (cacheHit || storeHit) ? 0 :
			(atomKind == HNSW_GUIDANCE_KIND_EXACT ? cache->buildMs :
			 (atomKind == HNSW_GUIDANCE_KIND_PAGE ? cache->pageBuildMs : cache->bloomBuildMs));
			if (cacheHit)
			{
	...
	if (nextGuidance.atoms == 0 || fragmentBytes <= 0)
	{
		hnsw_adaptive_profile.rejections++;
		hnsw_adaptive_profile.eventSequence++;
		descriptor->adaptiveBytes = fragmentBytes;
	...
	if (fragmentBytes > (int64) hnsw_metadata_cache_max_mb * 1024L * 1024L)
	{
		hnsw_adaptive_profile.rejections++;
		hnsw_adaptive_profile.eventSequence++;
		descriptor->adaptiveBytes = fragmentBytes;
	...
	descriptor->adaptiveState = nextGuidance.kind == HNSW_GUIDANCE_KIND_EXACT ?
		HNSW_ADAPTIVE_EXACT : (nextGuidance.kind == HNSW_GUIDANCE_KIND_PAGE ?
		HNSW_ADAPTIVE_PAGE : HNSW_ADAPTIVE_BLOOM);
	descriptor->adaptiveTargetKind = nextGuidance.kind;
	descriptor->adaptiveRefinePending = false;
	descriptor->adaptiveBytes = fragmentBytes;
	...
	{
		Relation	heapRel = table_open(heapOid, AccessShareLock);
		double		totalRows = heapRel->rd_rel->reltuples;

		table_close(heapRel, AccessShareLock);
		if (totalRows > 0 && nextGuidance.lastCacheRows > 0)
			descriptor->adaptiveExactSkipEst =
				Max(descriptor->adaptiveExactSkipEst,
					1.0 - ((double) nextGuidance.lastCacheRows / totalRows));
	}
						...
						int64 guidanceSkips, double heapFetchMs, double totalScanMs)
{
...

	{
		...
		if (descriptor->adaptiveState == HNSW_ADAPTIVE_PAGE)
		{
			...
			for (int i = 0; i < hnsw_active_guidance.atoms; i++)
			{
				int64		matching = hnsw_active_guidance.atom[i].cache != NULL ?
					hnsw_active_guidance.atom[i].cache->pageRows : 0;

				if (totalRows > 0 && matching > 0)
					exactEst = Max(exactEst, 1.0 - ((double) matching / totalRows));
			}
			hnsw_last_page_regret = exactEst - skipRate;
			descriptor->adaptiveExactSkipEst = exactEst;
			descriptor->adaptiveRefinePending = false;
		}
	}
...
vector_hnsw_guidance_activate(PG_FUNCTION_ARGS)
{
	...
	hnsw_page_compose_upgraded = false;
	if (!adaptive)
		kind = HnswGuidanceApplyComposeKind(kind,
			HnswGuidanceCountAtoms(filterDatums, filterNulls, filterCount));
...
vector_hnsw_guidance_profile(PG_FUNCTION_ARGS)
{
	...
	appendStringInfo(&output,
					 ...
					 "\"fragment_store_hits\":" INT64_FORMAT ","
						 ...
						 "\"compose_complete_only\":%s,"
						 "\"page_compose_upgraded\":%s,"
						 "\"page_regret\":%.6f,"
						 "\"marginal_delta\":%.6f}",
					 ...
					 hnsw_active_guidance.fragmentBuilds,
						 ...
						 hnsw_d3_compose_complete_only ? "true" : "false",
						 hnsw_page_compose_upgraded ? "true" : "false",
						 hnsw_last_page_regret,
						 hnsw_last_marginal_delta);
