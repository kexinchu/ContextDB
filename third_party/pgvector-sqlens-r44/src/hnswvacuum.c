...
RepairGraphElement(HnswVacuumState * vacuumstate, HnswElement element, HnswElement entryPoint)
{
	...
	Size		ntupSize = HnswNeighborTupleSize(element->level, m, acornBuild);
	char	   *base = NULL;
	...
	/* Init fields */
	HnswInitNeighbors(base, element, m, acornBuild, NULL);
	element->heaptidsLength = 0;

	/* Find neighbors for element, skipping itself */
	HnswFindElementNeighbors(base, element, entryPoint, index, support, m, efConstruction, true, acornBuild);
	...
	HnswSetNeighborTuple(base, ntup, element, m, acornBuild);
