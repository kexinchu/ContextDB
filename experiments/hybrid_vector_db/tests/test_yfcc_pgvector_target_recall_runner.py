...
from experiments.hybrid_vector_db.scripts import yfcc_pgvector_target_recall_runner as runner


class YfccFormalRunnerTests(unittest.TestCase):
    ...
    def test_manifest_audit_fails_closed_without_lcb_confirmation(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            ...
            for path in (selected, final):
                with path.open("w", newline="", encoding="utf-8") as output:
                    ...
                    writer.writeheader()
                    writer.writerow(row)
            manifest.write_text(json.dumps(self._payload(selected, final)), encoding="utf-8")
            ...
            self.assertTrue(any("LCB95" in error for error in audit["errors"]))

    def test_manifest_audit_accepts_a_bound_lcb_confirmed_slice(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            selected = root / "selected.csv"
            final = root / "final.csv"
            ...
            with mock.patch.object(
                ...
                audit = runner.audit_formal_manifest(manifest, args)
            self.assertTrue(audit["formal_complete"])
