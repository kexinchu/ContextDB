...
out_csv="${out_dir}/laion_target099_per_filter_paired_q5k.csv"
mkdir -p "${out_dir}"
...
exec "${python_bin}" \
  ...
  --mode-configs-json experiments/hybrid_vector_db/configs/table6_r41_laion_target099_per_filter_modes.json \
  ...
  --d3-fragment-store-namespace table6-r41-laion-t099-per-filter-q5k-retry1 \
  --d3-probe-requests 2 \
  ...
  --checkpoint-every-workload-requests 25 \
