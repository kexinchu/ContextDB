#!/usr/bin/env bash
# q1K screen: Attributes / Facts / Catalog / ACL × grocery_helpful
# vs stock, SQLens, and FAISS (allow-list time included). Not paper-eligible.
set -euo pipefail
...
flock 9
{
  echo "FIG5_ALLOWLIST_SCREEN_EXECUTE:$(date -Is)"
  "$python" "$runner" --execute --out-dir "$out_dir"
  echo "FIG5_ALLOWLIST_SCREEN_EXIT:$? $(date -Is)"
} 2>&1 | tee -a "$log"
