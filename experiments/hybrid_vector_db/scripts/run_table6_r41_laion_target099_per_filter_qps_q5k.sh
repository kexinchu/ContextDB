...
exec sudo -n --preserve-env=HOME,PGHOST,PGPORT,PGDATABASE,PGUSER,PGPASSWORD,PYTHONUNBUFFERED \
  "${python_bin}" \
  ...
  --config-id table6-r41-laion-t099-per-filter-q5k-c16 \
  --pair-id table6-r41-laion-t099-per-filter-q5k-c16 \
  --target-recall 0.99 \
  --stock-ef-search 50000 \
  --sqlens-ef-search 50000 \
  ...
  --sqlens-guided-collect-target 50000 \
  --stock-traversal-guided-target 10 \
  --sqlens-traversal-guided-target 500 \
  --stock-traversal-guided-burst 1 \
  --sqlens-traversal-guided-burst 8 \
  --filter-ef-search-json experiments/hybrid_vector_db/configs/table6_r41_laion_target099_per_filter_ef.json \
  --filter-traversal-target-json experiments/hybrid_vector_db/configs/table6_r41_laion_target099_per_filter_targets.json \
  ...
  --run-id "table6-r41-laion-t099-per-filter-q5k-c16-${run_suffix}" \
  --out-prefix "${out_dir}/laion_target099_per_filter_q5k_c16_${run_suffix}" \
  --overwrite \
  --execute
