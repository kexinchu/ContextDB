...
SPEC = DatasetSpec(
    ...
    index="public.yfcc10m_pgvector_embedding_hnsw",
    guidance_meta_table="public.yfcc10m_pgvector_guidance_meta",
    query_id_column="qid",
    ...
    default_filters_csv=ROOT / "results/hybrid_vector_db/yfcc10m_matched_recall_filters_q180.csv",
    default_truth_csv=ROOT / "results/hybrid_vector_db/yfcc10m_matched_recall_exact_truth_q180.csv",
    truth_builder_command=(
        "env",
        "OOD_ANNS_DATA={ood_anns_data}",
        "{python}",
        "experiments/hybrid_vector_db/scripts/yfcc10m_overlap_workload_truth.py",
        "--formal-matched-recall",
        "--filters-in",
        "results/hybrid_vector_db/yfcc10m_overlap_selectivity14_selected_q100_20260716.csv",
        "--queries-per-filter",
        "180",
        "--calibration-queries",
        "80",
        "--selected-out",
        "results/hybrid_vector_db/yfcc10m_matched_recall_selected_q180.csv",
        "--filters-out",
        "{filters_csv}",
        "--truth-out",
        "{truth_csv}",
...
def validate_formal_args(argv: list[str] | None = None) -> Any:
    """Constrain the generic executor to the preregistered YFCC formal protocol."""
...
def _audit_provenance(payload: dict[str, Any]) -> list[str]:
    ...
    if query_contract.get("query_table") != SPEC.query_table:
        errors.append("run_spec query table differs from the YFCC query relation")
    ...
    if not _relation_name_matches(SPEC.query_table, query_table.get("name")):
        errors.append("database provenance omits the bound YFCC query table")
    return errors
